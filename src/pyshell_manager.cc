...
PyShellManager::PyShellManager(ConstArrayView<EnvVarDesc> builtin_env_vars)
    : m_env_vars{builtin_env_vars}
{
    ...
    else // Get a guaranteed to be python binary
    {
        ...
        {
            auto candidate = format("{}/kakpython", dir);
                  ...
                  Func setup_child)
{
    ...
    Vector<const char*> execparams = { "python", "-c", cmdlinezstr };
    if (not params.empty())
...
Vector<String> generate_env(StringView cmdline, const Context& context, const ShellContext& shell__context)
{
    ...
    for (auto&& match : RegexIterator{cmdline.begin(), cmdline.end(), re})
    {
        ...
        auto var_it = shell__context.env_vars.find(name);
        try
        {
            String value = var_it != shell__context.env_vars.end() ?

                var_it->value : join(PyShellManager::instance().get_val(name, context) | transform(quoter(quoting)), ' ', false);
...
std::pair<String, int> PyShellManager::eval(
    StringView cmdline, const Context& context, StringView input,
    Flags flags, const ShellContext& shell__context)
{
    ...
    auto kak_env = generate_env(cmdline, context, shell__context);

    auto spawn_time = profile ? Clock::now() : Clock::time_point{};

    Pipe child_stdin{not input.empty()}, child_stdout, child_stderr;
    pid_t pid = spawn_shell(m_shell.c_str(), cmdline, shell__context.params, kak_env,
...
Vector<String> PyShellManager::get_val(StringView name, const Context& context) const
{
...
CandidateList PyShellManager::complete_env_var(StringView prefix,
